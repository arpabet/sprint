...
func WithEncodedECDSAPublicKey(pubPEM string) seal.SealerOption {
	return sealerOptionFunc(func(opt *seal.SealerOptions) error {
		...
		if err != nil {
			return xerrors.Errorf("parse EC public key, %v", err)
		}
...
func WithEncodedECDSAPrivateKey(privPEM string) seal.SealerOption {
	return sealerOptionFunc(func(opt *seal.SealerOptions) error {
		...
		if err != nil {
			return xerrors.Errorf("parse EC private key, %v", err)
		}
		opt.Algorithm = "ecdsa"
		opt.PrivateKey = key
		return nil
...
func ECDSASigner(opt *seal.SealerOptions) (seal.AsymmetricSigner, error) {
	...
	if opt.PublicKey != nil {
		...
		if !ok {
			return nil, xerrors.Errorf("not a ECDSA public key, %v", reflect.TypeOf(opt.PublicKey))
		}
	}
	if opt.PrivateKey != nil {
		...
		if !ok {
			return nil, xerrors.Errorf("not a ECDSA private key, %v", reflect.TypeOf(opt.PrivateKey))
		}
	}
...
func ECDSASignerIssue(bits int) (seal.AsymmetricSigner, error) {
	...
	case 521:
		c = elliptic.P521()
	default:
		return nil, xerrors.Errorf("unsupported curve bits %d", bits)
	}

...
func (t *implRunNode) Run(args []string) (err error) {
	...
	var coreContext glue.ChildContainer

	for _, child := range t.Children {
		if child.ChildName() == sprint.CoreRole {
			coreContext = child
			break
		}
	}
...
func findZapLogger(core glue.Container) (*zap.Logger, bool) {
	list := core.Bean(sprint.ZapLogClass, 0)
	if len(list) > 0 {

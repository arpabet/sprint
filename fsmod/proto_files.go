...
func (r *protoStreamReader) ReadTo(message proto.Message) error {
	...
	if err != nil {
		...
		return xerrors.Errorf("wrong number read %d, expected %d", n, len(lenBuf))
	}
	...
	if err != nil {
		...
		return xerrors.Errorf("wrong read bytes %d expected %d", n, len(block))
	}
...
func protobufWrite(w io.Writer, message proto.Message) ([]byte, error) {
	...
	if err != nil {
		return nil, xerrors.Errorf("proto marshal error, %v", err)
	}
	...
	if n, err := w.Write(lenBuf); err != nil {
		...
		return blob, xerrors.Errorf("wrong number written %d, expected %d", n, len(lenBuf))
	}

	if n, err := w.Write(blob); err != nil {
		return blob, err
	} else if n != len(blob) {
		return blob, xerrors.Errorf("wrong number written %d, expected %d", n, len(blob))
	}
